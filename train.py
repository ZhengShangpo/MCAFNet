...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    for k, v in model.named_parameters():
        import re
        pattern = re.compile(r'model\.10\.pos_emb|model\.10\.trans_blocks.sim\.*|model.11.CAM_SAM.SFT.B|model\.16\.CCNet\.CCA\.gamma')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.image_weights:
            ...
            if rank in [-1, 0]:
                ...
                dataset.indices = random.choices(range(dataset.n_rgb), weights=iw, k=dataset.n_rgb)  # rand weighted idx
            # Broadcast if DDP
            if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='models/yaml/MCAFNet.yaml', help='model.yaml path')
