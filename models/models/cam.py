...
class _ContextAttentionModule(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=2, mode='dot_product', sub_sample=True,
                 bn_layer=True):
    ...
    def _dot_product(self, xi, xj):
        ...
        y = y.view(batch_size, self.inter_channels, *xi.size()[2:])
        W_y = self.W(
            y)  # conv_nd(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1, stride=1, padding=0)
        z = W_y + xi
...
class Dilated_Context_Attention_Sum(nn.Module):
    ...
    def __init__(self, padding_dilation, input_channels):
        ...
        self.branch0 = nn.Conv2d(input_channels, input_channels, kernel_size=3, stride=1, padding=padding_dilation,
